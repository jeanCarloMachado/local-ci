#!/usr/bin/env python3
import os
from changes_configs.remove_latest_images import  import config
...
class BulkChange():
    ...
    def commit(self, repo_folder):
        os.system(f"cd {repo_folder} && git add . && git commit -m '{self.config['commit_message']}' ")

    def reset_changes(self, repo_folder):
        os.system(f"cd {repo_folder} && (git checkout master || git checkout main) && git fetch && git reset --hard HEAD && git clean -f -d")

    def create_pr(self, repo_name):
        repo_folder = self._projects_folder() + '/' + repo_name
        import subprocess;
        output = subprocess.getoutput(f'cd {repo_folder} &&  run_function current_branch')

        if output != self.config['branch_name']:
            raise Exception(f"Not in the right branch. Current: {output}, while should: {self.config['branch_name']}")

        os.system(f"cd {repo_folder} && git add . && gh pr create --fill --body '{self.config['pr_message']}' ")
